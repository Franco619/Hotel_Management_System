@php
	$id = Auth::user()->id;
	$profData = App\Models\User::find($id);
@endphp
...
<img src="{{(!empty($profData->photo)) ? url('upload/admin_images/'.$profData->photo) : url('upload/no_image.jpg')}}" class="user-img" alt="user avatar">
<div class="user-info">
<p class="user-name mb-0">{{ $profData->name }}</p>
<p class="designattion mb-0">{{$profData->email }}</p>
</div>
</a>
<ul class="dropdown-menu dropdown-menu-end">
